...
if(process.env.mysqlDb)
{
    ...
    connection.connect();
    const res = await query("CREATE TABLE IF NOT EXISTS `users` (`id` int(11) unsigned NOT NULL AUTO_INCREMENT,`email` varchar(255) NOT NULL,`password` varchar(255) NOT NULL,`rank` int(1) unsigned NOT NULL DEFAULT '0',KEY `Index 1` (`id`)) ENGINE=InnoDB DEFAULT CHARSET=latin1;");

    if(!res) throw "Could not create table";
}
